...
class TextAnalyzerGUI(ctk.CTk):
    ...
    def create_layout(self):
        ...
        btn_cargar = ctk.CTkButton(frame_botones, text="📂 Cargar archivo", command=self.cargar_archivo)
        btn_cargar.grid(row=0, column=0, padx=10)

        btn_analizar = ctk.CTkButton(frame_botones, text="🔎 Analizar texto", command=self.analizar_texto)
        btn_analizar.grid(row=0, column=1, padx=10)

        btn_palabra = ctk.CTkButton(frame_botones, text="🧠 Analizar palabra", command=self.analizar_palabra)
        btn_palabra.grid(row=0, column=2, padx=10)

        btn_exportar = ctk.CTkButton(frame_botones, text="💾 Exportar", command=self.exportar)
        btn_exportar.grid(row=0, column=3, padx=10)

        btn_salir = ctk.CTkButton(frame_botones, text="❌ Salir", fg_color="red", hover_color="#8B0000", command=self.salir)
        ...
        label_result = ctk.CTkLabel(frame_resultados, text="📈 Resultados del análisis", font=("Arial", 16, "bold"))
    ...
    # =========================
    def exportar(self):
        contenido = self.result_box.get("1.0", "end").rstrip()
        if not contenido:
            return
